...
def parse_args(argv=None):
    # --test の場合 argv.testに指定したものが代入できる
    ...
    parser.add_argument("--is_tsumo", default=False, type=str2bool,
                        help="Whether to tsumo")
    parser.add_argument("--is_riichi", default=False, type=str2bool,
                        help="Whether to riichi")

    parser.set_defaults(no_bar=False, display=False, resume=False, output_coco_json=False, output_web_json=False, shuffle=False,
                        benchmark=False, no_sort=False, no_hash=False, mask_proto_debug=False, crop=True, detect=False, display_fps=False,
                        emulate_playback=False, is_tsumo=False, is_riichi=False)
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    if len(classes) == args.top_k:
        ...
        mahjong_score = mahjong_calculation(classes, boxes, args.is_tsumo, args.is_riichi)
        print(mahjong_score)
        
        score_font = cv2.FONT_HERSHEY_DUPLEX
        max_score_w = -1
        for index in mahjong_score:
            score_w, _ = cv2.getTextSize(index, score_font, 2.0, 2)[0]
            if max_score_w < score_w:
                max_score_w = score_w
        score_w = max_score_w
        
        # 点数表示のためのテキストボックス
        # 背景
        score_bg = img_numpy.copy()
        pt1 = (int(img_numpy.shape[1] - score_w - 20), 0)
        pt2 = (img_numpy.shape[1], int(img_numpy.shape[0] * 0.25))
        bg_color = [255, 255, 255]
        cv2.rectangle(score_bg, pt1, pt2, bg_color, -1)
        img_numpy = cv2.addWeighted(score_bg, 0.4, img_numpy, 1 - 0.4, 0)
        # 文字
        offset_x = int(img_numpy.shape[1] - score_w)
        initial_y = 0
        dy = int(img_numpy.shape[0] / 20)
        score_color = [0, 0, 0]
        mahjong_score = [mahjong_score] if type(mahjong_score) == str else mahjong_score
        for i, text in enumerate(mahjong_score):
            offset_y = initial_y + (i + 1) * dy + 10
            cv2.putText(img_numpy, text, (offset_x, offset_y), score_font,
                        2.0, score_color, 2, cv2.LINE_AA)
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        net = Yolact()
        print('train_model',args.trained_model,type(args.trained_model))
